...
✅ 이미 마크 창에 있는 필드를 마크의 색상으로 다시 드래그 & 드롭
> 누적된 구간을 색깔별로 구분할 수 있음
...
```
???
테이블(아래로) : 필드 우클릭 > 다음을 사용하여 계산 > 테이블(아래로)설정 > 레이블 복사
...
![이중축설정x](../image/2주차/image.png)

* 측정값(매출,수익)을 행에 두었을 경우 (이중축설정O)
![alt text](../image/2주차/image(이중축설정).png)
```
측정값을 행에 두고 이중축을 설정할 경우, 
하나의 뷰에서 두개의 측정값을 쉽게 비교&확인할 수 있다. 
```
