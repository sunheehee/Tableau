...
```
-  병합된 셀, 메모, 바닥글, 빈 셀등을 감지하고, 이를 무시해 데이터 집합의 실제 필드와 값을 식별할 수 있게 하는 기능

- 데이터 관리시 태블로를 읽기 쉽도록 관리하는 경우에 사용
```
...
-크거나 복잡한 데이터 집합에 대해 빠른 데이터 수집 및 분석쿼리 처리 ㄱㄴ
-현재 데이터 원본의 상태를 hyper형태로 저장하는 것
-> 오프라인상황에서도 추출파일을 갖고 있으면 작업 ㄱㄴ 
-> 라이브에 비해 처리속도 빠름
-> 변경된 경우, 새로 고침을 통해 추출데이터를 최신화할 수 있음. 


=> 사용하고자 하는 데이터 원본의 저장위치 및 데이터 크기, 혹은 온오프라인 환경등을 고려해서 연결방식을 선택할 것. 
...
```
전체 주문 데이터 중에서도 반품된 주문을 확인하고 싶은 것이기에 주문 테이블과 반품 테이블에서 일치하는 데이터값만 알아보기 위해 INNER JOIN을 선택함.
